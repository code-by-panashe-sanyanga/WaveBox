...
A small music player web app. FastAPI serves track metadata from a JSON file, and a plain HTML/CSS/JavaScript page lists the tracks and plays audio through the browser. After StreamFlix in Node I wanted the same kind of catalogue-and-play flow in Python — same `fetch` + DOM pattern, different stack.

Not a full Spotify clone. Runs locally with a handful of demo tracks.
...
| File | What it does |
|------|----------------|
| **static/index.html** | Layout: now-playing panel, searchable library, genre filters, and a fixed player bar with prev/next and seek. |
| **static/style.css** | Dark blue theme, artwork placeholders per genre, active-row highlight, responsive grid. |
| **static/app.js** | Fetches `/api/tracks`, filters by genre/search, custom play/pause/seek controls, and queue navigation within the filtered list. |
...
Each entry in `tracks.json` needs these fields:

- `id` — unique number (used in the API and to highlight the active row)
- `title`, `artist`, `genre` — shown in the library and now-playing panel
- `url` — direct link to an MP3 the browser can stream (SoundHelix demos need internet)
...
- No shuffle or user accounts — prev/next only walks the current filtered list.
- Audio depends on external URLs and an internet connection.
- Artwork is CSS gradients, not real album covers.
